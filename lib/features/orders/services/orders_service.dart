      ...
      for (final odooOrder in fetchedOrders) {
        ...
        if (localOnly != null) {
          ...
          if (localOnly.stage != FsmOrderStage.draft ||
            ...
            final index =
                cleanOrders.indexWhere((o) => o.odooId == odooOrder.odooId);
            if (index != -1) {
  ...
  Future<OdooCreateResult?> _createOrderOnOdoo(FsmOrder order) async {
    ...
    if (order.recurringId != null &&
      ...
      try {
        ...
        logger.w(
            'Idempotency check failed with custom fields, retrying with basic fields...',
            error: e);
        try {
          ...
          if (existFallback.isNotEmpty) {
            final existingId = existFallback.first['id'] as int;
            logger.i(
                'Idempotency check fallback: Order already exists. Reusing ID: $existingId');
            return OdooCreateResult(existingId);
          }
        } catch (fallbackError) {
          logger.e('Idempotency check fallback failed entirely',
              error: fallbackError);
          if (fallbackError is OdooConnectionException ||
              fallbackError is OdooAuthException) {
            rethrow;
          }
        }
      }
