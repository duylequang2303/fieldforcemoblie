...
class MockOdooForStock implements OdooSessionManager {
  ...
  int step = 0; // Track state machine steps
...
void main() {
  group('StockService - Full State Machine Test', () {
    late Isar isar;
    ...
    test('Luồng State Machine Đầy Đủ (6 bước)', () async {
      ...
      // 3. Verify RPC calls (6 bước)
      expect(mockOdoo.methodCalls, [
        'stock.picking.type.search_read',  // 1a
        'stock.warehouse.read',            // 1b
        'stock.picking.create',            // 1c
        'stock.move.create',               // 1d
        'stock.picking.action_confirm',    // 2
        'stock.picking.action_assign',     // 3
        'stock.picking.read',              // 3 - check state
        'stock.move.write',                // 4
        'stock.picking.button_validate'    // 4
      ]);
    ...
    test('Thiếu warehouse_id quăng OdooBusinessException', () async {
      final order = FsmOrder()
        ..odooId = 100
        ..name = 'WO/TEST/001'
        ..stageId = 1
      ...
      expect(
        () => service.recordStockOut(orderOdooId: 100, productId: 123, productName: 'Ốc', qty: 10),
        throwsA(isA<OdooBusinessException>())
      );
    });

    test('Thiếu inventory_location_id quăng OdooBusinessException', () async {
      final order = FsmOrder()
        ..odooId = 100
        ..name = 'WO/TEST/001'
        ..stageId = 1
